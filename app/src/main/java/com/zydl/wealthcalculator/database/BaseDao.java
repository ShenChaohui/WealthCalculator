...
public abstract class BaseDao<T, Integer> {
    ...
    public void deleteTable(String tableName) throws SQLException {
        mDatabaseHelper.getWritableDatabase().execSQL("DELETE FROM " + tableName);
    }
     ...
     * @throws SQLException SQLException异常
     */
    public T queryT(String[] columnNames, Object[] columnValues) throws SQLException {
        List<T> lst = query(columnNames, columnValues);
        if (null != lst && !lst.isEmpty()) {
            return lst.get(0);
        } else {
            return null;
